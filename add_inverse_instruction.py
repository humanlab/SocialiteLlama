...
dataset = load_dataset("hlab/SocialiteInstructions", split="train")
...
dataset = dataset.add_column("Inverse Instruction", new_column)
dataset = dataset.map(add_inverse_column, with_indices=True)
print(dataset[0]["Inverse Instruction"])
...
new_column = ["Foo"] * len(dataset)
dataset = dataset.add_column("Article", new_column)
dataset = dataset.map(add_article_column, with_indices=True)
print(dataset[13]["Article"])
